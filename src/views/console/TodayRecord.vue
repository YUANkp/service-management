...
<script lang="ts" setup>
  ...
  const exportData = () => {
    ...
    exportXlsx(_columns, filterData.value, 'TodayRecord');
  };
  ...
  const saveData = () => {
    window.localStorage.setItem('TodayRecordData', JSON.stringify(sourceData.value));
  };

  const loadData = () => {
    const localData = JSON.parse(window.localStorage.TodayRecordData || '[]');
